...
import { 
  Cpu, Key, ArrowRight, Bell, Thermometer, Droplets, Sprout, Sun, 
  Power, LayoutDashboard, BarChart2, Settings, LogOut, Loader2, RefreshCw, Smartphone, Trash2, Calendar, Clock
} from "lucide-react";
...
export default function Home() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (cleanToken.length < 4) {
      setLoginError("Token minimal 4 karakter.");
      return;
    }
    setIsLogingIn(true);
    setLoginError("");
    ...
    try {
      ...
      startFirebaseListener(cleanToken);
    ...
    <>
      ...
      {screen === "splash" && (
        <div id="splash-screen" className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-950 transition-all duration-700">
          ...
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Amaranggana<span className="text-emerald-500">Farm</span></h1>
          <p className="text-slate-400 mt-2 text-sm tracking-widest uppercase">IoT Dashboard System</p>
        </div>
      )}

      {/* 2. LOGIN / AUTH SCREEN */}
      <div id="auth-screen" className={`${screen === "auth" ? "flex" : "hidden"} min-h-screen flex-col items-center justify-center p-6 relative z-40`}>
        <div className="glass-panel p-8 rounded-3xl shadow-2xl w-full max-w-sm text-center fade-in">
          <div className="w-16 h-16 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-6 shadow-[0_0_20px_rgba(16,185,129,0.2)] relative">
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="radiation-wave bg-emerald-500/20 w-full h-full"></div>
            </div>
            <Cpu className="text-emerald-400 w-8 h-8 relative z-10" />
          </div>
          <h2 className="text-2xl font-bold mb-2">Hubungkan Alat</h2>
          <p className="text-slate-400 text-sm mb-8">Masukkan token unik dari ESP32 Anda.</p>
          <form id="login-form" className="space-y-4" onSubmit={handleLogin}>
            <div className="relative">
              <Key className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-500 w-5 h-5" />
              <input type="text" required placeholder="Token (contoh: dev123)" value={token} onChange={e => setToken(e.target.value)}
            ...
            <button type="submit" disabled={isLogingIn} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 disabled:opacity-70">
              {isLogingIn ? <><Loader2 className="w-5 h-5 animate-spin" /> Menghubungkan...</> : <><span>Masuk ke Dashboard</span><ArrowRight className="w-5 h-5" /></>}
            </button>
          </form>
          {loginError && <p className="text-rose-400 text-sm mt-4">{loginError}</p>}
        </div>
      </div>

      {/* 3. MAIN DASHBOARD */}
      <div id="app-screen" className={`${screen === "app" ? "flex" : "hidden"} flex-col min-h-screen max-w-lg mx-auto w-full relative`}>
        ...
        <main className="flex-1 px-6 fade-in space-y-6 overflow-y-auto no-scrollbar pb-32">
          {currentTab === "home" && (
            <HomeTab sensorData={sensorData} activeToken={activeToken} handlePumpToggle={handlePumpToggle} />
          )}
          {currentTab === "stats" && (
            <StatsTab activeToken={activeToken} />
          )}
          {currentTab === "system" && (
            <SystemTab activeToken={activeToken} />
          )}
        </main>
...
// TAB: BERANDA (HOME)
// ---------------------------------------------------------
function HomeTab({ sensorData, activeToken, handlePumpToggle }: any) {
  return (
    <>
      {/* Sensor Cards Grid */}
      ...
      <div className="text-center mt-4">
        <p className="text-xs text-slate-500">Token Aktif: <span className="font-mono text-emerald-400">{activeToken}</span></p>
      </div>
    </>
  );
}
...
function StatsTab({ activeToken }: { activeToken: string }) {
  ...
  // Create deterministic mock data based on the chosen date and time to act as historical values
  const mockData = useMemo(() => {
    const data = [];
    // Seed using standard sum of string chars to ensure repeated views yield the exact same graph
    ...
    // Create 10 stepped points representing past historical jumps
    for (let i = 9; i >= 0; i--) {
      const timePoint = format(subHours(new Date(`${selectedDate}T${selectedTime}`), i), 'HH:mm');
      // Create random looking fluctuations but determininstic per seed
  ...
  // Handle Hydration issue with Recharts by deferring render slightly
  ...
  return (
    <div className="space-y-6">
          ...
          {isMounted && (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={mockData} margin={{ top: 5, right: 20, left: -20, bottom: 0 }}>
                ...
                {/* Stepped Line Chart Configuration */}
      ...
      <div className="glass-panel p-4 rounded-xl flex items-center gap-3 bg-blue-500/10 border-blue-500/20">
        <div className="p-2 bg-blue-500/20 rounded-full"><BarChart2 className="w-4 h-4 text-blue-400" /></div>
...
// TAB: SISTEM (SYSTEM)
// ---------------------------------------------------------
function SystemTab({ activeToken }: { activeToken: string }) {
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-5 rounded-2xl space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-800 col-span-2 flex justify-between items-center">
            ...
            <div className="px-2 py-1 bg-emerald-500/20 text-emerald-400 text-xs rounded-full border border-emerald-500/20 font-medium">Bagus</div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel p-5 rounded-2xl space-y-4">
        ...
        <div className="space-y-3">
          <button onClick={() => handleAction("Sensor sedang dikalibrasi ulang...")} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 p-4 rounded-xl flex items-center justify-between transition-colors">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-blue-500/20 text-blue-400 rounded-lg"><RefreshCw className="w-5 h-5" /></div>
          ...
          <button onClick={() => handleAction("Berhasil membersihkan cache sistem.")} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 p-4 rounded-xl flex items-center justify-between transition-colors">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-amber-500/20 text-amber-400 rounded-lg"><Trash2 className="w-5 h-5" /></div>
              <div className="text-left">
          ...
          <button onClick={() => handleAction("Melakukan proses reboot pada perangkat...")} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 border border-rose-900/50 p-4 rounded-xl flex items-center justify-between transition-colors group">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-rose-500/20 text-rose-400 rounded-lg group-hover:bg-rose-500 group-hover:text-white transition-colors"><Power className="w-5 h-5" /></div>
              <div className="text-left">
